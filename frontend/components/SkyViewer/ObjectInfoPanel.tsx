...
function ObjectInfoPanelContent({ info }: { info: ObjectInfo }) {
  ...
  const [hiddenAliases, setHiddenAliases] = useState<string[]>([]);

  const aliases = useMemo(
    () => info.aliases.filter((name) => !hiddenAliases.includes(name)),
    [hiddenAliases, info.aliases]
  );
          ...
          {aliases.length > 0 ? (
            aliases.map((alias) => (
              <button
                ...
                onClick={() => setHiddenAliases((current) => [...current, alias])}
                title="이 별칭 숨기기"
              >
                <span>{alias}</span>
