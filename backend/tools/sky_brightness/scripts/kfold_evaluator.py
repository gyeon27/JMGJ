...
def main() -> None:
    ...
    print(f"MSE : {np.mean(mses):.5f} 짹 {np.std(mses):.5f}")
    print(f"RMSE: {np.mean(rmses):.5f} 짹 {np.std(rmses):.5f}")
    print(f"Bias: {np.mean(biases):+.5f} 짹 {np.std(biases):.5f}")
    print(f"MAE : {np.mean(maes):.5f} 짹 {np.std(maes):.5f}")
